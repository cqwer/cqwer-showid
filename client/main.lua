...
Citizen.CreateThread(function()
    TriggerServerEvent("cqwer-showid:add-id")
    while true do
        Citizen.Wait(1)
        if IsControlPressed(0, cqwer.key) or forceDraw then
...
RegisterNetEvent('cqwer-showid:client:add-id')
AddEventHandler('cqwer-showid:client:add-id', function(identifier, playerSource)
...
RegisterCommand(cqwer.commandName, function()
...
function GetNeareastPlayers()
    ...
    if IsPedInAnyVehicle(playerPed, false) then
        ...
        local players, _ = GetPlayersInArea(playerCoords, cqwer.drawDistance)
